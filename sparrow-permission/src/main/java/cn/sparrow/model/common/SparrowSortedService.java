...
public class SparrowSortedService<T, ID> implements ISparrowSortedService<T, ID> {
	...
	public void sort(List<T> list) {
		...
		list.forEach(f -> {
			int index = sortLinkedList.indexOf(f);
			sortLinkedList.add(index == sortLinkedList.size() ? index : index + 1, map.get(this.getNextNodeId(f)));
	...
	public void setNextNodeId(T node, ID nextNodeId) {
		try {
			node.getClass().getDeclaredMethod("setNextNodeId", getNodeId(node).getClass()).invoke(node, nextNodeId);
	...
	public void setPreviousNodeId(T node, ID previousNodeId) {
		try {
			node.getClass().getDeclaredMethod("setPreviousNodeId", getNodeId(node).getClass()).invoke(node,
					previousNodeId);
		} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException | NoSuchMethodException
				| SecurityException e) {
			// TODO Auto-generated catch block
