...
public class OrganizationService {
  ...
  public List<OrganizationRole> getOrganizationRoles(@NotBlank String organizationId) {
    List<OrganizationRole> roles =
        organizationRoleRepository.findByIdOrganizationId(organizationId);
    roles.forEach(f -> {
  ...
  public List<OrganizationPositionLevel> getOrganizationLevels(@NotBlank String organizationId) {
    List<OrganizationPositionLevel> roles =
        organizationLevelRepository.findByIdOrganizationId(organizationId);
    roles.forEach(f -> {
  ...
  public List<Organization> getChildren(String parentId) {
    ...
    organizationRelations.forEach(f -> {
      ...
      f.getOrganization().setChildCount(
          organizationRelationRepository.countByIdParentId(f.getOrganization().getId()));
      f.getOrganization().setRoleCount(
          organizationRoleRepository.countByIdOrganizationId(f.getOrganization().getId()));
      f.getOrganization().setLevelCount(
          organizationLevelRepository.countByIdOrganizationId(f.getOrganization().getId()));
      f.getOrganization().setGroupCount(
          organizationGroupRepository.countByIdOrganizationId(f.getOrganization().getId()));
      f.getOrganization()
          .setEmployeeCount(employeeRepository.countByOrganizationId(f.getOrganization().getId()));
    ...
    organizationRelations.forEach(f -> {
      if (f.getId().getParentId().equals("-1")) {
        Organization rootOrganization =
            organizationRepository.getOne(f.getId().getOrganizationId());
